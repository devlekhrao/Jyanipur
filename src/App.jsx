  ...
  if (isLoggedIn) {
    return (
      <div className="flex w-screen h-screen overflow-hidden bg-zinc-50 font-['Poppins'] text-zinc-900 selection:bg-amber-100">
        ...
        {/* EDGE-TO-EDGE SIDEBAR (TEAK COLOR) */}
        <aside className="print:hidden w-[260px] h-full bg-[#B45309] text-white flex flex-col flex-shrink-0 z-20 shadow-2xl">            
            ...
            {navigationGroups.map((group) => (
              <div key={group.title} className="flex flex-col mb-2">
                <button onClick={() => toggleGroup(group.title)} className="flex items-center justify-between px-3 py-2 w-full text-left cursor-pointer group/nav rounded-lg hover:bg-white/10 transition-colors">
                  <span className="text-[10px] font-black text-amber-200/70 uppercase tracking-widest">{group.title}</span>
                  <svg className={`w-3.5 h-3.5 text-amber-200/50 transition-transform ${expandedGroups[group.title] ? 'rotate-180' : ''}`} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={3}><path strokeLinecap="round" strokeLinejoin="round" d="M19 9l-7 7-7-7" /></svg>
                </button>
                {expandedGroups[group.title] && (
                  <div className="flex flex-col gap-1 mt-1">
                    {group.pages.map((page) => (
                      <button key={page} onClick={() => handlePageSwitch(page)} className={`text-left px-3 py-2.5 rounded-xl text-xs transition-all duration-200 flex items-center ${activePage === page ? 'bg-white/20 text-white font-bold translate-x-1' : 'text-amber-100/80 hover:bg-white/10 hover:text-white cursor-pointer'}`}>
                        {page}
          ...
          <div className="p-4 bg-[#92400E] flex flex-col gap-2">
            <button onClick={() => handlePageSwitch('Settings')} className={`w-full py-3 rounded-xl text-[10px] uppercase font-bold cursor-pointer ${activePage === 'Settings' ? 'bg-white text-[#B45309]' : 'bg-white/10 text-white hover:bg-white/20'}`}>Settings</button>
