...
export default function Purchases() {
  ...
  const handleSaveBill = async (e) => {
    ...
    try {
      // 1. Save the Purchase Bill
      await savePurchase(payload);

      // 2. Automatically push items to RateBook
        ...
        <table className="w-full text-left border-collapse whitespace-nowrap min-w-[1000px]">
          <thead>
            <tr className="text-zinc-400 text-[9px] uppercase tracking-widest border-b border-zinc-200">
              ...
              <th className="py-3 px-2 font-semibold min-w-[200px]">Vendor Name</th>
          ...
          <tbody className="text-sm text-zinc-700">
            ...
            {loading ? (
              <tr><td colSpan="10" className="py-12 text-center text-zinc-400 font-medium text-xs">Loading purchases...</td></tr>
            ) : filteredPurchases.length === 0 ? (
              <tr><td colSpan="10" className="py-12 text-center text-zinc-400 font-medium text-xs">No purchases found. Click '+ Log Purchase Bill' above.</td></tr>
            ) : (
              filteredPurchases.map(p => (
                <React.Fragment key={p.id}>
                  <tr className={`border-b border-zinc-100 hover:bg-zinc-50/50 transition-colors group cursor-pointer ${expandedRows.includes(p.id) ? 'bg-zinc-50' : ''}`} onClick={() => toggleRow(p.id)}>
                    <td className="py-3 px-2 text-center text-zinc-400 font-bold text-xs">{expandedRows.includes(p.id) ? 'v' : '>'}</td>
                    <td className="py-3 px-2 font-medium text-zinc-600 text-xs">{p.invoiceDate}</td>
                    <td className="py-3 px-2 font-semibold text-zinc-800 text-xs">{p.invoiceNo || '-'}</td>
                    <td className="py-3 px-2 font-semibold text-zinc-800 text-xs truncate max-w-[200px]">{p.vendorName}</td>
                    <td className="py-3 px-2 text-zinc-400 font-mono text-[9px]">{p.gstin || 'UNREGISTERED'}</td>
                    <td className="py-3 px-2 text-right font-medium text-zinc-600 text-xs">₹ {p.taxableAmount.toLocaleString('en-IN', {minimumFractionDigits: 2})}</td>
                    <td className="py-3 px-2 text-right text-emerald-600 font-semibold text-xs">₹ {p.gstAmount.toLocaleString('en-IN', {minimumFractionDigits: 2})}</td>
                    <td className="py-3 px-2 text-right font-bold text-zinc-900 text-xs">₹ {p.totalAmount.toLocaleString('en-IN', {minimumFractionDigits: 2})}</td>
                  ...
                  {expandedRows.includes(p.id) && (
                    <tr className="bg-zinc-50 border-b border-zinc-200">
                      ...
                      <td colSpan="9" className="py-4 pr-6">
                        <div className="bg-white border border-zinc-200 rounded-xl p-4 shadow-inner">
                          <h4 className="text-[10px] font-bold text-zinc-400 uppercase tracking-widest mb-3">Itemized Details (Auto-Synced to RateBook)</h4>
                              ...
                              <tbody className="text-xs">
                                {p.items.map((item, idx) => (
                                  <tr key={idx} className="border-b border-zinc-50 last:border-0">
                                    ...
                                    <td className="py-2 text-right text-zinc-500">{item.gstPercent}%</td>
                                    <td className="py-2 text-right font-semibold text-zinc-800">₹{item.total?.toFixed(2)}</td>
                                  </tr>
                                ))}
                              </tbody>
                            </table>
                          ) : (
                            <p className="text-xs text-zinc-400 italic">No line items recorded for this bill.</p>
                          )}
                        </div>
                      </td>
                    </tr>
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-zinc-900/60 backdrop-blur-sm">
          <div className="bg-white w-full max-w-5xl max-h-[90vh] rounded-[2rem] shadow-2xl overflow-hidden flex flex-col">
            ...
            {/* Modal Header */}
            <div className="bg-zinc-900 px-8 py-5 flex justify-between items-center text-white shrink-0">
              <div>
            ...
            <div className="flex-1 overflow-y-auto p-8 custom-scrollbar bg-zinc-50/50">
              ...
              <div className="bg-white p-6 rounded-2xl border border-zinc-200 shadow-sm mb-6">
                <h3 className="text-[10px] font-black text-zinc-800 uppercase tracking-widest mb-4">1. Vendor Details</h3>
                    ...
                    <tbody className="divide-y divide-zinc-100">
                      {enhancedItems.map((item, index) => (
                        <tr key={index}>
                          ...
                          <td className="py-2 pr-2">
                            <select value={item.gstPercent} onChange={e => updateItemRow(index, 'gstPercent', e.target.value)} className={modalInputClass}>
            ...
            {/* Modal Footer */}
            <div className="bg-white border-t border-zinc-200 px-8 py-5 flex justify-between items-center shrink-0">
              <div className="flex gap-8">
                ...
                <div>
                  <span className="block text-[9px] font-bold text-emerald-500 uppercase tracking-widest">Input Tax (ITC)</span>
