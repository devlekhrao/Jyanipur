  ...
  return (
    ...
    <div className="w-full h-full overflow-y-auto pb-32 flex flex-col space-y-6 [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
      ...
      <div className="bg-[#1E3A8A] border border-blue-800 p-6 rounded-[2rem] shadow-lg text-white relative overflow-hidden shrink-0">
        ...
        <p className="text-3xl font-semibold text-[11px] tracking-tight">₹ {metrics.availableFunds.toLocaleString('en-IN')}</p>
      </div>
      ...
      <div className="flex gap-4 overflow-x-auto [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none] -mx-3 px-3 pb-2 shrink-0">
        <div className="bg-white border border-zinc-200 p-5 rounded-[1.5rem] shadow-sm min-w-[150px] shrink-0">
          <span className="text-[9px] font-bold text-zinc-400 uppercase tracking-widest block mb-1">Received</span>
          <p className="text-xl font-semibold text-[11px] text-emerald-600">₹ {metrics.totalReceived.toLocaleString('en-IN')}</p>
        </div>
        <div className="bg-white border border-zinc-200 p-5 rounded-[1.5rem] shadow-sm min-w-[150px] shrink-0">
          <span className="text-[9px] font-bold text-zinc-400 uppercase tracking-widest block mb-1">Pending Invoices</span>
          <p className="text-xl font-semibold text-[11px] text-[#1E3A8A]">₹ {metrics.pendingInvoices.toLocaleString('en-IN')}</p>
        </div>
        <div className="bg-white border border-zinc-200 p-5 rounded-[1.5rem] shadow-sm min-w-[150px] shrink-0">
          <span className="text-[9px] font-bold text-zinc-400 uppercase tracking-widest block mb-1">Unpaid Purchases</span>
          <p className="text-xl font-semibold text-[11px] text-red-500">₹ {metrics.unpaidPurchases.toLocaleString('en-IN')}</p>
        </div>
      </div>
      ...
      <div className="bg-white border border-zinc-200 p-6 rounded-[2rem] shadow-sm flex flex-col items-center justify-between shrink-0">
        <div className="w-full flex justify-between items-center mb-6">
          <h3 className="text-xs font-bold text-zinc-900 uppercase tracking-widest">Cash Distribution</h3>
        </div>
      ...
      <div className="bg-white border border-zinc-200 p-6 rounded-[2rem] shadow-sm flex flex-col shrink-0">
        <h3 className="text-xs font-bold text-zinc-900 uppercase tracking-widest mb-6">Revenue vs Cost</h3>
      ...
      <div className="bg-white border border-zinc-200 p-6 rounded-[2rem] shadow-sm shrink-0">
        <div className="flex justify-between items-center mb-5">
          <h3 className="text-xs font-bold text-zinc-900 uppercase tracking-widest">Active Projects</h3>
      ...
      <div className="bg-white border border-zinc-200 p-6 rounded-[2rem] shadow-sm shrink-0">
        <div className="flex justify-between items-center mb-5">
          ...
          <span className="bg-red-50 text-red-600 text-[10px] font-semibold text-[11px] px-2.5 py-1 rounded-lg border border-red-100">{metrics.urgentPayables.length}</span>
        </div>
          ...
          <div className="space-y-3">
            {metrics.urgentPayables.map((v, idx) => (
              <div key={idx} className="flex justify-between items-center bg-zinc-50 p-4 rounded-2xl border border-zinc-100 active:scale-95 transition-transform">
                <div>
                  <p className="text-xs font-bold text-zinc-900 max-w-[120px] truncate">{v.vendorName || v.name}</p>
                  <p className="text-[10px] font-bold text-zinc-400 uppercase tracking-widest mt-0.5">Pending Dues</p>
                </div>
                <div className="text-right">
                  <span className="text-sm font-semibold text-[11px] text-red-500 block">₹{Number(v.balance).toLocaleString('en-IN')}</span>
                  <button 
                    onClick={() => setActiveTab('Vendors')} 
                    className="text-[9px] font-bold text-[#1E3A8A] uppercase tracking-wider mt-1 cursor-pointer"
                  >
                    Resolve &rarr;
                  </button>
                </div>
