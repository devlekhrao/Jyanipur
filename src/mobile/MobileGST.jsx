    ...
    <div className="w-full h-full flex flex-col font-['Poppins']">
      ...
      <div className="mb-3 shrink-0">
        <div className="flex justify-between items-center mb-2">
          <div>
            <h2 className="text-2xl font-bold text-zinc-900 tracking-tight">GST & Filing</h2>
      ...
      <div className="space-y-2 mb-3 shrink-0">
        <div className="bg-zinc-900 text-white p-4 rounded-2xl shadow-md flex justify-between items-center">
          <div>
            <span className="text-[8px] font-semibold text-[11px] text-amber-500 uppercase tracking-widest block">Net Tax Payable (3B)</span>
            <p className="text-xl font-semibold text-[11px] mt-0.5">₹ {netPayable.total.toLocaleString('en-IN')}</p>
          </div>
        ...
        <div className="grid grid-cols-2 gap-2">
          <div className="bg-white p-3 rounded-2xl border border-zinc-200 shadow-sm">
            <span className="text-[8px] font-semibold text-[11px] text-zinc-400 uppercase tracking-widest block">Outward Tax (1)</span>
            <p className="text-base font-semibold text-[11px] text-zinc-900 mt-0.5">₹ {taxData.outward.total.toLocaleString('en-IN')}</p>
          </div>
          <div className="bg-emerald-50 p-3 rounded-2xl border border-emerald-100 shadow-sm">
            <span className="text-[8px] font-semibold text-[11px] text-emerald-600 uppercase tracking-widest block">ITC Credit (2B)</span>
            <p className="text-base font-semibold text-[11px] text-emerald-700 mt-0.5">₹ {taxData.inward.total.toLocaleString('en-IN')}</p>
          </div>
        </div>
      </div>
      ...
      <div className="mb-3 shrink-0">
        {activeTab === 'GSTR-1 (Sales)' && (
          <button 
            ...
            disabled={syncing} 
            className="w-full py-3.5 bg-[#1E3A8A] hover:bg-blue-900 text-white font-semibold text-[11px] rounded-xl text-xs uppercase tracking-wider shadow-md active:scale-95 transition-transform"
          >
        ...
        {activeTab === 'GSTR-2B (Purchases)' && (
          <button 
            ...
            disabled={syncing} 
            className="w-full py-3.5 bg-emerald-600 hover:bg-emerald-500 text-white font-semibold text-[11px] rounded-xl text-xs uppercase tracking-wider shadow-md active:scale-95 transition-transform"
          >
        ...
        {activeTab === 'GSTR-3B' && (
          <button 
            ...
            disabled={syncing} 
            className="w-full py-3.5 bg-amber-500 hover:bg-amber-400 text-zinc-900 font-semibold text-[11px] rounded-xl text-xs uppercase tracking-wider shadow-md active:scale-95 transition-transform"
          >
      ...
      <div className="flex-1 overflow-y-auto space-y-3 pb-4 [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
        {[
          ...
          <div key={idx} className="bg-white border border-zinc-200 rounded-[1.5rem] p-4 shadow-sm space-y-3">
            <h4 className="font-bold text-zinc-900 text-sm border-b border-zinc-100 pb-2">{item.title}</h4>
            
            <div className="grid grid-cols-3 gap-2 text-center">
              <div className="bg-zinc-50 p-2 rounded-xl">
                <span className="text-[8px] font-semibold text-[11px] text-zinc-400 uppercase block">Sales Tax</span>
                <p className="text-xs font-semibold text-[11px] text-zinc-800 mt-0.5">₹ {item.outward.toLocaleString('en-IN')}</p>
              </div>

              <div className="bg-emerald-50 p-2 rounded-xl">
                <span className="text-[8px] font-semibold text-[11px] text-emerald-600 uppercase block">ITC Credit</span>
                <p className="text-xs font-semibold text-[11px] text-emerald-700 mt-0.5">₹ {item.inward.toLocaleString('en-IN')}</p>
              </div>

              <div className="bg-blue-50 p-2 rounded-xl">
                <span className="text-[8px] font-semibold text-[11px] text-[#1E3A8A] uppercase block">Net Due</span>
                <p className="text-xs font-semibold text-[11px] text-[#1E3A8A] mt-0.5">₹ {item.net.toLocaleString('en-IN')}</p>
              </div>
            </div>
          </div>
        ))}
