    ...
    return (
      <div className="w-full h-full font-['Poppins'] flex flex-col items-center justify-center">
        <div className="bg-white p-10 rounded-[2.5rem] border border-zinc-200 shadow-xl max-w-md w-full text-center">
          ...
          <h2 className="text-xl font-extrabold text-zinc-900 mb-2">Project P&L Reports</h2>
  ...
  return (
    <div className="w-full h-full font-['Poppins'] flex flex-col">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end pb-4 border-b border-zinc-200 mb-6 gap-4 shrink-0">
        <div>
          <h2 className="text-2xl font-extrabold text-zinc-900 tracking-tight">Financial Overview</h2>
        ...
        <div className="flex-1 overflow-y-auto space-y-6 [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
            <div className="bg-white border border-zinc-200 p-6 rounded-[2rem] shadow-sm">
              <span className="text-[9px] font-extrabold text-zinc-400 uppercase tracking-widest block mb-1">Contract Budget</span>
              <p className="text-2xl font-black text-zinc-900">₹ {pnlData.budget.toLocaleString('en-IN')}</p>
            </div>
            <div className="bg-emerald-50/70 border border-emerald-100 p-6 rounded-[2rem] shadow-sm">
              <span className="text-[9px] font-extrabold text-emerald-600 uppercase tracking-widest block mb-1">Income Received</span>
              <p className="text-2xl font-black text-emerald-700">₹ {pnlData.incomeReceived.toLocaleString('en-IN')}</p>
            </div>
            <div className="bg-red-50/70 border border-red-100 p-6 rounded-[2rem] shadow-sm">
              <span className="text-[9px] font-extrabold text-red-500 uppercase tracking-widest block mb-1">Total Project Cost</span>
              <p className="text-2xl font-black text-red-600">₹ {pnlData.totalCost.toLocaleString('en-IN')}</p>
            </div>
            <div className={`p-6 rounded-[2rem] shadow-lg border ${pnlData.netProfit >= 0 ? 'bg-zinc-900 border-zinc-900 text-white' : 'bg-red-600 border-red-600 text-white'}`}>
              <span className="text-[9px] font-extrabold uppercase tracking-widest block mb-1 opacity-80">Current Net Profit</span>
              <p className="text-2xl font-black">₹ {pnlData.netProfit.toLocaleString('en-IN')}</p>
            </div>
          </div>
          ...
          <div className="bg-white border border-zinc-200 p-6 rounded-[2rem] shadow-sm">
            <div className="flex justify-between items-end mb-4">
              <h3 className="text-xs font-extrabold text-zinc-900 uppercase tracking-widest">Profit Margin Health</h3>
              <span className={`text-xl font-black ${pnlData.profitMargin >= 15 ? 'text-emerald-600' : pnlData.profitMargin > 0 ? 'text-amber-500' : 'text-red-500'}`}>
          ...
          <div className="bg-white border border-zinc-200 rounded-[2rem] shadow-sm overflow-hidden mb-6">
            <div className="p-5 border-b border-zinc-100 bg-zinc-50/50">
              <h3 className="text-xs font-extrabold text-zinc-900 uppercase tracking-widest">Cost Breakdown Analysis</h3>
            </div>
            <table className="w-full text-left text-xs">
              <tbody className="divide-y divide-zinc-100 text-zinc-800">
                <tr className="hover:bg-zinc-50 transition-colors">
                  <td className="py-4 px-6 font-semibold text-zinc-700">Subcontractor Payouts (Advances + Bills)</td>
                  <td className="py-4 px-6 text-right font-extrabold text-zinc-900">₹ {pnlData.subCost.toLocaleString('en-IN')}</td>
                </tr>
                <tr className="hover:bg-zinc-50 transition-colors">
                  <td className="py-4 px-6 font-semibold text-zinc-700">Site Petty Cash Expenses</td>
                  <td className="py-4 px-6 text-right font-extrabold text-zinc-900">₹ {pnlData.pettyCost.toLocaleString('en-IN')}</td>
                </tr>
                <tr className="bg-zinc-50 border-t-2 border-zinc-200">
                  <td className="py-4 px-6 font-black text-zinc-900 text-right uppercase tracking-widest text-xs">Total Direct Cost :</td>
                  <td className="py-4 px-6 text-right font-black text-red-600 text-base">₹ {pnlData.totalCost.toLocaleString('en-IN')}</td>
                </tr>
              </tbody>
            </table>
          </div>
